...
export const Multiple: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Multiple selection enabled select allowing to pick multiple options.',
      },
    },
  },
  play: async ({ canvasElement }) => {
    ...
    const selectedOptions = Array.from(select.selectedOptions).map((option: HTMLOptionElement) => option.value)
...
export const CustomIcon: Story = {
  render: (args) => (
    <SelectNative {...args} icon={<Icon name="chevrons-up-down" size={16} className="text-primary" />}>
