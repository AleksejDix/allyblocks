...
const ComboboxDemo = ({
  ...
  return (
    <Combobox open={open} onOpenChange={setOpen}>
      ...
      <ComboboxContent>
        <ComboboxInput placeholder="Search..." value={inputValue} onChange={(e) => setInputValue(e.target.value)} />
        <ComboboxList>
          <ComboboxEmpty>No results found.</ComboboxEmpty>
          <ComboboxGroup heading="Options">
            {options.map((option) => (
              <ComboboxItem key={option.value} value={option.value} onSelect={handleSelect}>
                ...
                {value === option.value && <Icon name="check" className="ml-auto h-4 w-4" />}
              </ComboboxItem>
            ))}
          </ComboboxGroup>
        </ComboboxList>
...
export const WithDescription: Story = {
  render: () => <ComboboxDemo options={countries} placeholder="Select a country" />,
}
