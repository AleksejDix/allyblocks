import { type ChangeEvent, type ClipboardEvent, type KeyboardEvent, type FocusEvent } from 'react'
 ...
 */
export function handleNumberKeyDown(e: KeyboardEvent<HTMLInputElement>, options: NumberValidationOptions = {}) {
...
>(
  ...
  const finalSanitized = !integerOnly ? sanitized.replace(/\.(?=.*\.)/g, '') : sanitized
  ...
  const newValue =
    currentValue.substring(0, cursorPos) + finalSanitized + currentValue.substring(inputEl.selectionEnd || cursorPos)
