  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-96">
        <FieldSelect name="fruit" label="Favorite Fruit" placeholder="Select a fruit" required>
...
function WithDefaultValueForm() {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-96">
        <FieldSelect name="city" label="Select Your City" placeholder="Where are you located?">
        ...
        <div className="text-sm text-muted-foreground">Default value: London (should be pre-selected)</div>
        <Button type="submit">Submit</Button>
      </form>
    </Form>
  )
...
function DisabledForm() {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-96">
        <FieldSelect name="fruit" label="Favorite Fruit" placeholder="Select a fruit" disabled>
          <SelectItem value="apple">Apple</SelectItem>
          <SelectItem value="banana">Banana</SelectItem>
          <SelectItem value="orange">Orange</SelectItem>
          <SelectItem value="grape">Grape</SelectItem>
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-96">
        ...
        <FieldSelect name="city" label="Your City" placeholder="Select a city" required>
          <SelectItem value="new-york">New York</SelectItem>
          <SelectItem value="san-francisco">San Francisco</SelectItem>
          <SelectItem value="london">London</SelectItem>
          <SelectItem value="tokyo">Tokyo</SelectItem>
...
export const MultipleSelects: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const fruitTrigger = buttons.find(
      (btn) => btn.getAttribute('id')?.includes('form-item') && btn.textContent?.includes('Banana'),
    )
    await expect(fruitTrigger).toHaveTextContent('Banana')

    // Check that city select shows placeholder
    const cityTrigger = buttons.find(
      (btn) => btn.getAttribute('id')?.includes('form-item') && btn.textContent?.includes('Select a city'),
    )
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-96">
        ...
        <div className="text-sm text-muted-foreground">Selected: {form.watch('tags')?.join(', ') || 'None'}</div>
        <Button type="submit">Submit</Button>
      </form>
    </Form>
  )
