...
export const Basic: Story = {
  render: () => {
    const handleAction = (type: string, payload?: Record<string, unknown>) => {
      notify(`${type} action triggered${payload ? ` with payload: ${JSON.stringify(payload)}` : ''}`)
    }
    ...
    return (
      <ActionMenu>
        ...
        <ActionMenuContent>
          ...
          <ActionMenuItem className="text-destructive" onAction={() => notify('Delete action triggered')}>
            <Icon name="trash" />
            Delete
          </ActionMenuItem>
        </ActionMenuContent>
        ...
        <ActionMenuContent>
          {itemActions.map((action, index) => (
            <ActionMenuItem
              ...
              disabled={action.props?.disabled as boolean}
              onAction={() => {
                notify(`${action.type} action triggered for item: ${action.payload?.id}`)
              }}
            >
              {action.before && <span>{action.before}</span>}
              {action.label}
...
export const WithRootActionHandler: Story = {
  render: () => {
    const handleValueChange = (value: string, _event: Event, context?: Record<string, unknown>) => {
      notify(`Root handler: ${value} action executed ${context ? `with context: ${JSON.stringify(context)}` : ''}`)
    }
    ...
    return (
      <ActionMenu onValueChange={handleValueChange}>
        ...
        <ActionMenuContent>
          ...
          <ActionMenuItem value="share" context={{ itemId: 'doc-123', permissions: 'read' }}>
            <Icon name="share" />
            Share Document
          </ActionMenuItem>
          <ActionMenuItem className="text-destructive" value="delete" context={{ itemId: 'doc-123' }}>
            <Icon name="trash" />
            Delete Document
          </ActionMenuItem>
        </ActionMenuContent>
    ...
    return (
      <div className="border rounded-md p-4">
        ...
        <ul className="space-y-2">
          {documents.map((doc) => (
            <li key={doc.id} className="flex items-center justify-between p-2 border-b">
              <div>
                <span className="font-medium">{doc.title}</span>
                <span className="ml-2 text-sm text-muted-foreground">({doc.type})</span>
              </div>
              <ActionMenu onValueChange={handleValueChange}>
                <ActionMenuTrigger asChild>
                ...
                <ActionMenuContent>
                  <ActionMenuItem value="document:edit" context={{ id: doc.id, type: doc.type }}>
                    <Icon name="edit" />
                    Edit
                  </ActionMenuItem>
                  <ActionMenuItem value="document:share" context={{ id: doc.id, type: doc.type, shareType: 'email' }}>
                    <Icon name="mail" />
                    Share via Email
                  </ActionMenuItem>
                  <ActionMenuItem value="document:share" context={{ id: doc.id, type: doc.type, shareType: 'link' }}>
                    <Icon name="link" />
                    Copy Link
                  </ActionMenuItem>
                  <ActionMenuItem className="text-destructive" value="document:delete" context={{ id: doc.id }}>
                    <Icon name="trash" />
                    Delete
                  </ActionMenuItem>
                </ActionMenuContent>
          ...
          <CardAction>
            <ActionGroup>
              <ActionSplit>
                ...
                <ActionMenu>
                  <ActionMenuTrigger asChild>
                    <IconButton variant="outline" aria-label="More export options">
                      <Icon name="chevron-down" />
                    </IconButton>
                  </ActionMenuTrigger>
                  <ActionMenuContent>
                    <ActionMenuItem onAction={() => notify('Exporting data from the last 24 hours')}>
                      <Icon name="clock" />
                      Last 24 hours
                    </ActionMenuItem>
                    <ActionMenuItem onAction={() => notify('Exporting data from the last 7 days')}>
                      <Icon name="calendar-days" />
                      Last 7 days
                    </ActionMenuItem>
                    <ActionMenuItem onAction={() => notify('Exporting data from the last 30 days')}>
                      <Icon name="calendar-range" />
                      Last 30 days
                    </ActionMenuItem>
                    <ActionMenuItem className="text-destructive" onAction={() => notify('Exporting all data')}>
                      <Icon name="database" />
                      All data
                    </ActionMenuItem>
                  </ActionMenuContent>
              ...
              <ActionMenu>
                ...
                <ActionMenuContent>
                  <ActionMenuCheckboxItem checked={true}>Name</ActionMenuCheckboxItem>
                  <ActionMenuCheckboxItem checked={true}>Email</ActionMenuCheckboxItem>
                  <ActionMenuCheckboxItem checked={true}>Role</ActionMenuCheckboxItem>
                </ActionMenuContent>
              </ActionMenu>
            </ActionGroup>
          </CardAction>
              ...
              <tr key={user.id} className="border-b">
                ...
                <td className="p-2 text-right">
                  <ActionMenu>
                    ...
                    <ActionMenuContent>
                      <ActionMenuItem onAction={() => handleAction('edit', user.id, user.name)}>
                        <Icon name="edit" />
                        Edit User
                      </ActionMenuItem>
                      <ActionMenuItem onAction={() => handleAction('permissions', user.id, user.name)}>
                        <Icon name="shield" />
                        Permissions
                      </ActionMenuItem>
                      <ActionMenuItem
                        className="text-destructive"
                        onAction={() => handleAction('delete', user.id, user.name)}
                      >
                        <Icon name="trash" />
                        Delete User
                      </ActionMenuItem>
    ...
    function AdvancedMenuDemo() {
      ...
      const handleValueChange = (value: string, _event: Event, context?: Record<string, unknown>) => {
        ...
        if (value === 'filter:toggle') {
          notify(`Global handler: Smart filtering ${context?.checked ? 'enabled' : 'disabled'}`)
        } else if (value === 'sort:change') {
          notify(`Global handler: Sort order changed to ${context?.radioValue}`)
        }
      }

      // Local action handlers
      const handleNotificationToggle = (checked: boolean) => {
        notify(`Local handler: Notifications ${checked ? 'enabled' : 'disabled'}`)
      }
      ...
      return (
        <ActionMenu onValueChange={handleValueChange}>
          ...
          <ActionMenuContent>
            ...
            <ActionMenuGroup>
              <ActionMenuItem value="refresh" onAction={() => notify('Refreshing items...')}>
                <Icon name="refresh-ccw" />
                Refresh
              </ActionMenuItem>
              <ActionMenuItem value="duplicate" onAction={() => notify('Duplicating...')}>
                <Icon name="copy" />
                Duplicate
              </ActionMenuItem>
            </ActionMenuGroup>
            ...
            <ActionMenuRadioGroup value={sortOrder} onValueChange={setSortOrder}>
              <ActionMenuRadioItem value="newest" onAction={() => notify(`Sorted by newest first`)}>
                Newest First
              </ActionMenuRadioItem>
              <ActionMenuRadioItem value="oldest" onAction={() => notify(`Sorted by oldest first`)}>
                Oldest First
              </ActionMenuRadioItem>
              <ActionMenuRadioItem value="alphabetical" onAction={() => notify(`Sorted alphabetically`)}>
                Alphabetical
              </ActionMenuRadioItem>
            </ActionMenuRadioGroup>
            ...
            <ActionMenuItem value="help" onAction={() => notify('Opening help...')}>
      ...
      return (
        <div className="space-y-6">
          ...
          <ActionSplit className="w-fit">
            ...
            <ActionMenu>
              <ActionMenuTrigger asChild>
                <IconButton variant="default" size="md" aria-label="More options">
                  <Icon name="chevron-down" />
                </IconButton>
              </ActionMenuTrigger>
              <ActionMenuContent>
                ...
                <ActionMenuSeparator />
                <ActionMenuItem className="text-destructive" onAction={handleDelete}>
                  <Icon name="trash" />
                  Delete
                </ActionMenuItem>
              </ActionMenuContent>
          ...
          <div className="flex gap-4">
            <ActionSplit variant="outline">
              <Button variant="outline" onClick={() => notify('Primary outline action')}>
                <Icon name="save" className="mr-2" />
                Save
              </Button>
              <ActionMenu>
                <ActionMenuTrigger asChild>
                  <IconButton variant="outline" size="md" aria-label="More options">
                    <Icon name="chevron-down" />
                  </IconButton>
                </ActionMenuTrigger>
                <ActionMenuContent>
            ...
            <ActionSplit variant="secondary">
              <Button variant="secondary" onClick={() => notify('Primary secondary action')}>
                <Icon name="save" className="mr-2" />
                Save
              </Button>
              <ActionMenu>
