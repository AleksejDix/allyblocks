...
import type { ActionToolbarRootProps, ActionToolbarButtonProps } from './ActionToolbar.types'
import { ActionToolbarContext, useActionToolbar } from './ActionToolbar.context'

/**
 * ActionToolbar - A flexible toolbar with action support
 */
export function ActionToolbar({ children, className, onValueChange, ...props }: ActionToolbarRootProps) {
  const { action, setAction } = useActionProvider()

  return (
    <ActionToolbarContext.Provider value={{ action, setAction, onValueChange }}>
      <div className={cn('flex items-center space-x-2 bg-muted/20 p-2 rounded-md', className)} {...props}>
        {children}
      </div>
 ...
 */
export function ActionToolbarContent({ children, className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  const actionContext = useActionToolbar()
  const handleAnimationEnd = useActionHandler(actionContext)

  return (
    <div className={cn('flex-1', className)} onAnimationEnd={handleAnimationEnd} {...props}>
      {children}
    </div>
  )
}
