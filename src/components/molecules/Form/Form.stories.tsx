  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-[400px]">
        ...
        <FieldCheckbox name="checkbox" label="Accept terms" description="Checkbox input example" required />
        ...
        <FieldOTP name="otp" label="OTP Input" description="One-time password input" maxLength={6} required />
...
export const Default: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const usernameError = canvas.getByText(/username must be at least 2 characters/i)
    await expect(usernameError).toBeVisible()

    const emailError = canvas.getByText(/please enter a valid email address/i)
    await expect(emailError).toBeVisible()

    const passwordError = canvas.getByText(/password must be at least 8 characters/i)
...
export const WithCustomValidation: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const feedbackTextarea = canvas.getByPlaceholderText(/tell us what you think/i)
    await userEvent.type(feedbackTextarea, 'This is a detailed feedback that is more than 20 characters long.')
